...
def main(input_path, force_field_type, output_path):
    ...
    try:
        ...
        for oe_molecule in input_stream.GetOEGraphMols():
            ...
            if force_field_type.lower() == "smirnoff":
                ...
                #if not os.path.exists("espaloma-0.3.0rc1.pt"):
                #    os.system("wget http://data.wangyq.net/espaloma_model.pt")
                # create an Espaloma Graph object to represent the molecule of interest
                # From Ken Takaba, Slack chat
                molecule_graph = esp.Graph(off_molecule)
                
                # apply a trained espaloma model to assign parameters
                espaloma_model = torch.load("espaloma-0.3.0rc1.pt") 
                espaloma_model.eval()   # 0.3.0 uses dropouts
                espaloma_model(molecule_graph.heterograph)
                # use espaloma charge jointly trained with energies and forces by specifying charge_method="nn"
                # note that espaloma charge is different from espaloma_charge (doi: 10.48550/ARXIV.2302.06758)
                omm_system = esp.graphs.deploy.openmm_system_from_graph(molecule_graph, forcefield="openff_unconstrained-2.0.0", charge_method="nn")
            else:
