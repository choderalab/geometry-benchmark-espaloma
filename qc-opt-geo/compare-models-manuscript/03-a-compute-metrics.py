...
def compare_two_mols(rmol, qmol, rmsd_cutoff=1.0):
    """
...
def _compute_metrics(
    label: str, input_dictionary: Dict[str, Any], index: int, conformer_rmsd_match: bool,
) -> pandas.DataFrame:
    ...
    for oe_qm_molecule, oe_mm_molecule in zip(
        ...
        if conformer_rmsd_match:
            qm_mol_indices = compare_two_mols(oe_qm_molecule, oe_mm_molecule)
            dde = [relative_mm_energies[indx] - relative_qm_energies[item] if item is not None else numpy.nan for indx, item in enumerate(qm_mol_indices) if item is not None]
        else:
...
@click.option(
    "--conformer_rmsd_match",
    "conformer_rmsd_match",
    type=click.BOOL,
    default=False,
)
def main(input_path, index, output_path, conformer_rmsd_match):
    ...
    metrics = pandas.concat(
        [_compute_metrics(label, input_dictionary, index, conformer_rmsd_match) for label in method_labels]
    )
